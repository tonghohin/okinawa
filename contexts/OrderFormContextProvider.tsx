...
export default function OrderFormContextProvider({ children }: { children: React.ReactNode }) {
    const [formDataContext, setFormDataContext] = useState<Order>(InitialStates.Order);

    useEffect(() => {
        setFormDataContext((prevOrderFormData) => ({ ...prevOrderFormData, total: Tools.getTotal(prevOrderFormData.items) }));
